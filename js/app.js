/* ══════════════════════════════════════════════════════
   House Guide — App JS
   Theme switcher · Checklist · Comments · Progress
══════════════════════════════════════════════════════ */

// ── THEMES ──────────────────────────────────────────
const THEMES = [
  { id: 'ivory', label: 'Warm Ivory',   dot: '#C8702A', tip: 'Warm Ivory & Terracotta (House Walls)' },
  { id: 'sage',  label: 'Sage Forest',  dot: '#3D7A50', tip: 'Sage Forest & Green (House Garden Wall)' },
  { id: 'rose',  label: 'Dusty Rose',   dot: '#A8685C', tip: 'Dusty Rose & Blush (House Pink Palette)' },
  { id: 'wheat', label: 'Golden Wheat', dot: '#C48A18', tip: 'Golden Wheat & Amber (House Warm Tones)' },
  { id: 'stone', label: 'Stone Modern', dot: '#4A6EA8', tip: 'Stone & Slate (House Study/Bath Palette)' },
];
...
function saveComment(key, val) {
  clearTimeout(_saveT[key]);
  _saveT[key] = setTimeout(() => {
...
function resetAll() {
  ...
  const keys = Object.keys(localStorage).filter(k => k.startsWith('hg_'));
  keys.forEach(k => localStorage.removeItem(k));
  location.reload();
}

// ── CONCEPT IMAGES ────────────────────────────────────
// Load contextual images from Unsplash into .concept-img elements
const IMAGE_MAP = {
  foundation:  'photo-1504307651254-35680f356dfd',
  concrete:    'photo-1590856029826-c7a73142bbf1',
  steel:       'photo-1558618666-fcd25c85cd64',
...
function loadConceptImages() {
  document.querySelectorAll('.concept-img[data-img]').forEach(div => {
    const key = div.dataset.img;
    const id  = IMAGE_MAP[key];
    if (!id) return;
...
function loadHomeDashboard() {
  ...
  pages.forEach(p => {
    ...
    if (cnt) cnt.textContent = data.done + ' of ' + data.total + ' items done (' + pct + '%)';
  });
}
...
window.addEventListener('DOMContentLoaded', () => {
  // Theme
  injectThemeSwitcher();
  const savedTheme = localStorage.getItem('hg_theme') || 'ivory';
